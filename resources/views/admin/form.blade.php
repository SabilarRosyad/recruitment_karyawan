            ...
            <div class="flex-none w-full px-3">
                <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
                    <div class="p-6 pb-0 mb-0 bg-white rounded-t-2xl">
                        <h6 class="text-lg font-bold text-slate-700">Form Edit Admin</h6>
                    </div>
                        ...
                        <div class="p-6 overflow-x-auto">
                            <form action="{{ isset($admin) ? route('admin.tambah.update', $admin->id) : route('admin.tambah.simpan') }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-300 focus:border-blue-500" id="username" name="username" value="{{ isset($admin) ? $admin->username : '' }}">
                                    @error('username')
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-300 focus:border-blue-500" id="email" name="email" value="{{ isset($admin) ? $admin->email : '' }}">
                                    @error('email')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    ...
                                    <input type="password" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-300 focus:border-blue-500" id="password" name="password" value="{{ isset($admin) ? $admin->password : '' }}">
                                    @error('password')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="mb-4">
                                    ...
                                    <select id="role" name="role" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-300 focus:border-blue-500">
                                        ...
                                        <option value="user" {{ isset($admin) && $admin->role == 'user' ? 'selected' : '' }}>User</option>
                                    </select>
                                    @error('role')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="mb-4">
                                    ...
                                    @if (isset($admin) && $admin->profile_photo)
                                        <img src="{{ asset('storage/'.$admin->profile_photo) }}" alt="Foto Profil" class="w-8 h-8 object-cover inline-block ml-2">
                                    @endif
                                </div>
                                <div>
